...
class TestStorageConfiguration:
    ...
    def test_storage_postgres_from_env(self):
        ...
        with patch.dict(os.environ, {"STORAGE_TYPE": "postgres", "DATABASE_URL": "postgresql://localhost/test"}):
            result = load_config_from_env(config)
...
class TestSchedulerConfiguration:
    ...
    def test_scheduler_redis_from_env(self):
        ...
        with patch.dict(os.environ, {"SCHEDULER_TYPE": "redis", "REDIS_URL": "redis://localhost:6379"}):
            result = load_config_from_env(config)
...
class TestSentryConfiguration:
    ...
    def test_sentry_enabled_from_env(self):
        ...
        with patch.dict(os.environ, {"SENTRY_ENABLED": "true", "SENTRY_DSN": "https://example.com/sentry"}):
            result = load_config_from_env(config)
    ...
    def test_sentry_enabled_variations(self):
        ...
        for value in ["true", "1", "yes", "True", "YES"]:
            with patch.dict(os.environ, {"SENTRY_ENABLED": value, "SENTRY_DSN": "https://example.com"}):
                result = load_config_from_env(config)
                assert result["sentry"]["enabled"] is True
...
class TestTelemetryConfiguration:
    ...
    def test_oltp_endpoint_from_env(self):
        ...
        with patch.dict(os.environ, {"TELEMETRY_ENABLED": "true", "OLTP_ENDPOINT": "http://localhost:4318"}):
            result = load_config_from_env(config)
    ...
    def test_oltp_not_loaded_when_telemetry_disabled(self):
        ...
        with patch.dict(os.environ, {"TELEMETRY_ENABLED": "false", "OLTP_ENDPOINT": "http://localhost:4318"}):
            result = load_config_from_env(config)
